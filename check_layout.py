...
def main():
    ...
    if config.get("project", {}).get("name") != "stat-tracker":
        print("Wrong root pyproject.toml: use the app configuration, not the camera package configuration.")
        return 1
    web_dependencies = config.get("tool", {}).get("flet", {}).get("web", {}).get("dependencies", [])
    if "flet-camera==0.86.5" not in web_dependencies or "stc-camera-preview" not in web_dependencies:
        print("The web build is missing its two camera dependencies.")
        return 1
