...
def _data_dir() -> str:
    ...
    if sys.platform == "win32":
        ...
        return d
    mobile_storage = os.getenv("FLET_APP_STORAGE_DATA")
    if mobile_storage:
        os.makedirs(mobile_storage, exist_ok=True)
...
SYNC_FILE = os.path.join(_data_dir(), "stattracker_sync.json")


def save_sync_state(payload):
    with _save_lock:
        _atomic_write_json(SYNC_FILE, payload)


def load_sync_state():
    if not os.path.exists(SYNC_FILE):
        return {}
    with open(SYNC_FILE, encoding="utf-8") as stream:
