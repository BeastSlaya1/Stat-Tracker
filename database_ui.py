...
class DatabaseSyncMixin(GameBrowserMixin, AccountManagementMixin):
    ...
    def _open_database_dialog(self, _=None):
        ...
        if self._can_manage_database():
            content += [ft.Button('Browse saved games',on_click=self._open_saved_games),ft.Button('Manage accounts',on_click=self._open_accounts),ft.Button('Manage shared database',on_click=self._open_catalog_manager)]
        if conflict_count:
