...
import base64, io, json, os, re, socket, sys, threading, time, urllib.request, urllib.error, uuid
...
from assets_data import ICON_SMALL_PNG_B64, SPLASH_PNG_B64
from database_ui import DatabaseSyncMixin
from camera_network import CameraNetworkMixin
...
# ── Reference data: opponent schools, age-group/team naming ──────────────────
# Bundled from Joshua's own Stat_Tracker_Database.accdb (Access) — as a
# plain Python literal here, not a file read from disk at runtime, and
# NOT a live connection to the .accdb itself. Access databases only open
# via a Windows-only ODBC driver (the "Access Database Engine"), which
# doesn't exist on Android or in a browser at all — a live connection
# was never going to work identically across Windows/Android/Web, so
# the actual reference *data* was pulled out and embedded here instead,
# which does. If the source database ever changes, re-export it and
# update these two literals — no schema/driver dependency either way.
KNOWN_OPPONENT_SCHOOLS = [
    "Maritzburg College", "Hilton", "Glenwood", "Clifton",
    "Michael House", "Northwood", "Durban Boys Highschool",
]  # St Charles College itself is excluded from this list — it's always
   # the fixed home team (see build_scc_team), never a selectable
   # opponent.
...
def school_quick_pick_row(text_field, page_ref, err_field=None, schools=None):
    """A row of small tap-to-fill chips for KNOWN_OPPONENT_SCHOOLS,
    sitting under an opponent-name TextField. Deliberately NOT a
    dropdown/autocomplete replacing the text field — plenty of fixtures
    are against schools that aren't in this fixed reference list at all
    (invitational tournaments, one-off friendlies, a school not yet
    added to the source database), so free typing has to keep working
    exactly as it already did; this only saves typing for the common
    case where the opponent is one of these known schools."""
    def _pick(school):
        def _handler(_):
            text_field.value = school
    ...
    chips = [
        ft.TextButton(
            school,
            style=ft.ButtonStyle(color=TEXT2, bgcolor=SURFACE2,
                                  shape=ft.RoundedRectangleBorder(radius=14),
                                  padding=ft.Padding.symmetric(horizontal=10, vertical=2)),
            on_click=_pick(school),
        )
        for school in (schools if schools is not None else KNOWN_OPPONENT_SCHOOLS)
    ]
    return ft.Row(chips, spacing=6, run_spacing=6, wrap=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
class StatTrackerApp(CameraNetworkMixin, DatabaseSyncMixin):
    ...
    def __init__(self, page: ft.Page):
        ...
        self._web_loaded = not self.is_web
        ...
        self.is_ios = self.page.platform == ft.PagePlatform.IOS
        ...
        self.camera_image = ft.Image(
            ...
            fit=ft.BoxFit.CONTAIN, border_radius=16, width=640, height=360,
        ...
        self.page.on_resize = self._on_layout_resize
    ...
    def _build_splash_screen(self, pct: float, label: str, first_paint: bool = False):
        ...
        """
        logo = ft.Image(
            src=f"data:image/png;base64,{SPLASH_PNG_B64}",
    ...
    async def _save_web_async(self):
        ...
        try:
            payload = json.dumps({
                ...
                "database": self._database_payload() if hasattr(self, "_database") else None,
            })
    ...
    def _layout_width(self):
        return float(getattr(self.page, "width", None) or 1200)

    async def _on_layout_resize(self, _):
        generation = getattr(self, "_resize_generation", 0) + 1
        self._resize_generation = generation
        await asyncio.sleep(0.15)
        if generation == self._resize_generation:
            self._fs_skeleton = None
            self._full_refresh()

    def _responsive_controls(self, control):
        if isinstance(control, (ft.Button, ft.TextButton, ft.IconButton)):
            return control
        if isinstance(control, ft.Row):
            control.controls = [item for item in control.controls
                if not (isinstance(item, ft.Container) and item.expand and item.content is None)]
            if not control.scroll and not any(getattr(item, "expand", False) for item in control.controls):
                control.wrap = True
        for child in getattr(control, "controls", []) or []:
            self._responsive_controls(child)
    ...
    def _build_navbar(self):
        brand = ft.Row([
            ...
            ft.Column([
                ft.Row([ft.Text("STAT TRACKER", size=15, color=TEXT, weight=ft.FontWeight.W_900),
                        pill("v4 Live", INDIGO),
                        self._logger_name_pill()], spacing=4, wrap=True),
                txt("Real-Time Tactical Logging & Analytics", size=9, color=MUTED2, mono=True),
            ], spacing=1, width=min(330, self._layout_width() - 108)),
        ], spacing=8)
        ...
        self.match_dd = ft.Dropdown(
            ...
            on_select=self._on_match_select, width=min(270, self._layout_width() - 48))
        ...
        return ft.Container(
            content=ft.Column([
                ...
                ft.Row([
                    ft.Button("Get the app", icon=ft.Icons.DOWNLOAD, visible=self.is_web,
                              url="https://stattrackerv4.stream/downloads/",
                              tooltip="Download for Android, Windows, iPhone and iPad"),
                    ft.Button("Database", icon=ft.Icons.CLOUD_SYNC,
    ...
    def _full_refresh(self):
        if not getattr(self, '_database_ready', True):
            self.page.controls=[ft.Container(content=ft.Text('Loading your account…'),padding=24)]
            self.page.update()
            return
    ...
    def _build_active_tab(self, m):
        ...
        if self.active_tab == "LOGGER":    return self._responsive_controls(self._build_logger_tab(m))
        if self.active_tab == "TIMELINE":  return self._responsive_controls(self._build_timeline_tab(m))
        if self.active_tab == "STATS":     return self._responsive_controls(self._build_stats_tab(m))
        return ft.Container()
    ...
    def _build_camera_mode_screen(self):
        ...
        if self._rtc_available():
            return self._build_network_camera_mode()
        if self.is_web:
            return ft.Text("Camera support is missing. Please update the website build.")
        ...
        return ft.Container(
            content=ft.Column([
                ...
                ft.Row([stream_btn, self._rotate_view_button(), self._mirror_view_button()], wrap=True, alignment=ft.MainAxisAlignment.CENTER),
        ...
        return ft.Container(
            content=ft.Column([
                ft.Row([
                    ...
                    ft.Container(expand=True),
                    cam_btn,
                    self._rotate_view_button(),
                    self._mirror_view_button(),
                    fullscreen_btn,
                ], spacing=8),
                ft.Stack([
                    ft.Container(content=self._video_display_widget(), bgcolor=BG,
    ...
    def _build_url_connect_row(self):
        ...
        discover_btn = ft.Button(
            content=ft.Row([
                ...
                ft.Text("Scanning…" if self.wireless_discovering else "Scan for cameras",
        ...
        if self.discovered_cameras:
            for cam in self.discovered_cameras:
                result_rows.append(ft.Container(
                    content=ft.Row([ft.Icon(ft.Icons.VIDEOCAM, size=14, color=EMERALD),
                                    ...
                                    ft.Container(expand=True),
                                    txt(cam.get("kind", "Local camera"), size=9, color=MUTED2)], spacing=8),
            ...
            result_rows.append(txt("No devices found yet — tap Scan for cameras while another "
                                   "device has Camera Mode → Start Streaming running.",
                                   size=9, color=MUTED2))

        return ft.Column([
            ft.Row([discover_btn], spacing=6),
            txt(getattr(self, "_camera_scan_message", ""), size=10, color=MUTED),
    ...
    def _build_fullscreen_logger(self, m: Match):
        if self._layout_width() < 900:
            ...
            try:
                panel = self._responsive_controls(self._build_logger_tab(m))
            finally:
                self.fullscreen_video = True
            return ft.Container(content=ft.Column([
                ft.Button("Exit fullscreen", icon=ft.Icons.FULLSCREEN_EXIT,
                          on_click=self._exit_fullscreen_video), panel,
            ], expand=True), padding=8, expand=True, bgcolor=BG)
        ...
        video_area = ft.Container(
            content=ft.Stack([
                ft.Container(content=self._video_display_widget(), bgcolor=BG, expand=True,
                             ...
                             alignment=ft.Alignment.CENTER),
                ft.Container(content=live_badge, padding=8),
                ft.Container(content=ft.Row([self._rotate_view_button(), self._mirror_view_button(), cam_toggle_btn],
                    spacing=6, tight=True), padding=8, alignment=ft.Alignment.TOP_RIGHT),
            ], expand=True),
            expand=True)
    ...
    def _build_category_grid(self, m: Match):
        ...
        return ft.GridView(controls=tiles, runs_count=columns,
                           child_aspect_ratio=tile_width / tile_height,
                           spacing=6, run_spacing=6, height=rows * (tile_height + 6))
    ...
    def _build_radar_chart_canvas(self, m: Match):
        ...
        width = min(340, max(210, self._layout_width() - 100))
    ...
    def _import_txt(self):
        if not self._database_token:
            self._snack('Sign in before importing matches.')
    ...
    def _start_camera(self, _=None):
        ...
        if self._rtc_available() and not (isinstance(source, str) and source.startswith(("http://", "https://"))):
            self._start_rtc_video("receive", source[6:]) if isinstance(source, str) and source.startswith("rtc://") else self._start_rtc_video("preview")
            return
        ...
        if isinstance(source, str) and source.startswith("native:"):
            ...
            # stc_camera_preview (this project's own custom Dart
            # extension — see its module docstring for why) now covers
            # BOTH roles: Inputter Mode's smooth local live view
            # (broadcasting=False) AND Camera Mode's actual broadcast
            # source (broadcasting=True, which additionally starts a
            # real continuous camera image stream on the Dart side and
            # feeds frame bytes back via on_frame/_on_stc_preview_frame —
            # genuine video framerate, not a still-photo slideshow).
            # Either way, zero Python involvement in rendering the local
            # preview itself. Nothing async to kick off here — the Dart
            # side handles camera selection/initialization/streaming
            # entirely on its own once the control is shown (see
            # _video_display_widget) with these properties set, driven
            # by _full_refresh() below the same way any other control's
            # property change is.
            if HAS_STC_CAMERA_PREVIEW:
                self.camera_on = True; self.camera_error = None
                self._camera_capture_stop.clear()
                ctrl = self._ensure_stc_preview_ctrl()
                ctrl.lens = lens
                ctrl.broadcasting = (self.app_mode == "camera")
                self._full_refresh(); return

            # Fallback only — stc_camera_preview isn't available in this
            # build for some reason. Uses flet-camera's take_picture(),
            # polled repeatedly. See
            # _native_camera_takepicture_poll_loop's docstring for why
            # take_picture() specifically (confirmed working, unlike
            # flet-camera's continuous-preview/streaming path) — but note
            # it's capped at roughly 2.8fps by the hardware photo
            # pipeline, well below what stc_camera_preview's image-stream
            # broadcasting achieves above.
    ...
    def _start_cv2_camera(self, source):
        ...
        if not cap.isOpened():
            ...
            self._full_refresh(); return
        # Ask the camera hardware for 60fps. This is only a request — a
        # given webcam/driver may cap it lower (many stock laptop webcams
        # top out at 30fps regardless), but where the hardware supports
        # it, the default OpenCV/driver setting is often 30fps even when
        # 60 is available, so explicitly asking is what actually unlocks
        # the higher rate.
        try:
    ...
    def _ensure_stc_preview_ctrl(self):
        ...
        if self._stc_preview_ctrl is None:
            self._stc_preview_ctrl = stcam.StcCameraPreview(
                ...
                on_frame=self._on_stc_preview_frame)
        return self._stc_preview_ctrl

    def _on_stc_preview_frame(self, e):
        """stc_camera_preview's on_frame — e.data is a plain base64-encoded
        JPEG string (see that control's docstring for why: plain string
        event, same reasoning as on_error), fired continuously from a
        real camera image stream on the Dart side once broadcasting=True
        is set, instead of flet-camera's take_picture()-polling fallback.

        Only feeds _latest_jpeg_frame (what the MJPEG server broadcasts
        to a paired Inputter Mode device) — there's nothing to do for
        the local on-screen preview here, since that's the
        StcCameraPreview widget itself rendering natively on the Dart
        side with zero Python involvement per frame, exactly like
        Inputter Mode's own local view already works."""
        try:
    ...
    def _on_stc_preview_error(self, e):
        ...
        Two distinct kinds of error share this one event: camera
        initialization failing outright (permission denied, no camera,
        hardware busy — the preview never starts at all, so camera_on
        gets turned back off), versus broadcasting-specific failures
        that the Dart side prefixes with "Camera broadcast failed:" (bad
        image format, repeated encode failures — see camera_preview.dart)
        where the local preview is working fine and should stay on;
        only the paired device's stream is actually broken."""
        msg = getattr(e, "data", None) or "Unknown camera error."
        if msg.startswith("Camera broadcast failed:"):
    ...
    def _rotate_view_button(self):
        return ft.Button("Rotate view", icon=ft.Icons.ROTATE_RIGHT,
                         on_click=self._rotate_video_view,
                         tooltip="Rotate this screen's video 90 degrees clockwise")

    def _mirror_view_button(self):
        return ft.Button("Mirror: On" if getattr(self, "_video_mirrored", False) else "Mirror view",
                         icon=ft.Icons.FLIP, on_click=self._mirror_video_view,
                         tooltip="Flip this screen's video left-to-right")

    def _mirror_video_view(self, event=None):
        self._video_mirrored = not getattr(self, "_video_mirrored", False)
        view = getattr(self, "_video_rotation_view", None)
        if view is not None:
            view.flip = ft.Flip(flip_x=self._video_mirrored)
        if event is not None and getattr(event, "control", None) is not None:
            event.control.content = "Mirror: On" if self._video_mirrored else "Mirror view"
        self.page.update()

    def _rotate_video_view(self, _=None):
        self._video_quarter_turns = (getattr(self, "_video_quarter_turns", 0) + 1) % 4
        view = getattr(self, "_video_rotation_view", None)
        if view is not None:
            view.quarter_turns = self._video_quarter_turns
        # Update the existing display, preserving the camera and pairing session.
        self.page.update()

    def _video_display_widget(self):
        """What to place in the video panel's Container.

        Any native/browser camera source — Inputter Mode's local view
        AND Camera Mode's broadcast source alike — shows
        stc_camera_preview (this project's own custom Dart extension,
        see packages/stc_camera_preview) directly, for genuine native
        frame rate with zero Python involvement per rendered frame. This
        is the one real exception to "always self.camera_image": that
        control renders itself directly (a live Dart-side CameraPreview
        widget). Camera Mode additionally sets broadcasting=True on it
        (see _start_camera) so it also produces frame bytes for the
        MJPEG server via on_frame — but the on-screen widget shown here
        is the same either way.

        Every other case — desktop OpenCV, wireless/IP camera URL, or
        the flet-camera take_picture() fallback when stc_camera_preview
        isn't available at all — uses the regular self.camera_image,
        same as before."""
        if self.camera_on and getattr(self, "_rtc_ctrl", None) is not None:
            content = self._rtc_ctrl
        elif (self.camera_on
                and isinstance(self.camera_source, str)
                and self.camera_source.startswith("native:")
                and HAS_STC_CAMERA_PREVIEW
                and self._stc_preview_ctrl is not None):
            content = self._stc_preview_ctrl
        else:
            content = self.camera_image
        view = getattr(self, "_video_rotation_view", None)
        if view is None:
            view = self._video_rotation_view = ft.RotatedBox()
        view.quarter_turns = getattr(self, "_video_quarter_turns", 0)
        view.content = content
        view.flip = ft.Flip(flip_x=getattr(self, "_video_mirrored", False))
        return view
    ...
    @staticmethod
    def _describe_pull_error(ex: Exception) -> str:
        """Turns a raw connection exception into something a non-technical
        person can actually act on, instead of the console-only print()
        this used to be limited to — which is invisible in a packaged
        app, and is exactly why a broken connection here showed up to
        the user as "nothing happens", not as any kind of error."""
        if isinstance(ex, urllib.error.HTTPError):
            if ex.code == 403:
                return ("The Camera Mode device rejected this connection "
                         "(pairing not approved). Try reconnecting.")
            return f"Camera device returned an error (HTTP {ex.code})."
        if isinstance(ex, urllib.error.URLError):
            reason = getattr(ex, "reason", ex)
            if isinstance(reason, (TimeoutError, socket.timeout)):
                return ("Timed out trying to reach the Camera Mode device — "
                         "check it's still on and on the same network.")
            if isinstance(reason, ConnectionRefusedError):
                return ("Connection refused — the Camera Mode device isn't "
                         "broadcasting on this address right now.")
            return f"Can't reach the Camera Mode device ({reason})."
        if isinstance(ex, (TimeoutError, socket.timeout)):
            return "Connected, but the video feed stopped responding (timed out)."
        return f"Camera stream error: {type(ex).__name__}: {ex}"
    ...
    def _mjpeg_url_pull_loop(self, url: str):
        ...
        Every failure path here now sets self.camera_error and refreshes
        the UI. It used to only print() to the console on exceptions and
        otherwise retry silently forever — invisible in a packaged app,
        which is exactly what made a broken connection look like nothing
        was happening at all rather than an actual, nameable error."""
        printed_error = False
        _MIN_UPDATE_INTERVAL = 1.0 / 60.0  # cap UI pushes at ~60fps
        _last_ui_update = 0.0
        _NO_FRAME_TIMEOUT = 6.0  # how long to wait after a successful HTTP
                                  # connection before treating "connected but
                                  # nothing's actually arriving" as its own
                                  # reportable problem, rather than staying
                                  # silent indefinitely.
        while not self._camera_capture_stop.is_set():
            try:
                req = urllib.request.Request(url, headers={"User-Agent": "StatTracker/4"})
                with urllib.request.urlopen(req, timeout=8) as resp:
                    # The TCP/HTTP connection itself succeeded — clear any
                    # previous "can't reach"-style error immediately, even
                    # though no frame has arrived yet. If nothing shows up
                    # within _NO_FRAME_TIMEOUT below, that gets its own
                    # distinct message instead of silence.
                    if self.camera_error is not None:
                        self.camera_error = None
                        self._full_refresh()
                    ...
                    while not self._camera_capture_stop.is_set():
                        ...
                        if newest_jpeg is not None:
                            ...
                            if now - _last_ui_update >= _MIN_UPDATE_INTERVAL:
                              ...
                              and time.monotonic() - connected_at > _NO_FRAME_TIMEOUT):
                            # Connected fine at the HTTP level, but nothing
                            # resembling a JPEG frame has shown up in
                            # _NO_FRAME_TIMEOUT seconds. This is exactly the
                            # "device connects but the stream silently does
                            # nothing" symptom — most likely the Camera Mode
                            # device's own camera/broadcast isn't actually
                            # producing frames (check its screen for an
                            # error there too), not a problem on this end.
                            msg = ("Connected to the camera device, but no video "
                                   "has arrived yet. Check the Camera Mode "
                                   "device's own screen for an error there.")
                            if self.camera_error != msg:
                                self.camera_error = msg
                                self._full_refresh()
                ...
                if not printed_error:
                    ...
                    printed_error = True
                msg = self._describe_pull_error(ex)
                if self.camera_error != msg:
                    self.camera_error = msg
                    self._full_refresh()
                if self._camera_capture_stop.is_set():
    ...
    def _stop_camera(self, _=None):
        if getattr(self, "_rtc_ctrl", None) is not None:
            self._rtc_ctrl = None
            self._rtc_state = {}
            self.camera_mode_streaming = False
        self.camera_on = False
        self._camera_capture_stop.set()
        if self._stc_preview_ctrl is not None:
            # This control is cached and reused across start/stop cycles
            # (see _ensure_stc_preview_ctrl's docstring) — reset
            # broadcasting explicitly so a later "off" -> "preview-only"
            # start (e.g. switching from Camera Mode to Inputter Mode on
            # the same device) doesn't inherit a stale True from last
            # time. _video_display_widget stops returning this control
            # the moment camera_on is False above, which is what
            # actually unmounts it and lets the Dart side's dispose()
            # tear the image stream down; this is just belt-and-suspenders
            # bookkeeping on the Python object itself.
            self._stc_preview_ctrl.broadcasting = False
        if self._cv2_cap:
    ...
    def _start_discovery_beacon(self):
        ...
        # iOS UDP broadcast requires a separately approved Apple entitlement.
        # Use the displayed stream URL and the normal pairing handshake instead.
        if self.is_ios:
            return
    ...
    def _scan_for_wireless_cameras(self, _=None):
        ...
        self._camera_scan_message = ""
        self._full_refresh()
        try:
            self.page.run_task(self._scan_camera_devices_async)
        except Exception:
            self.wireless_discovering = False
            self._camera_scan_message = "Could not start the scan. Please try again."
            self._full_refresh()

    def _connect_discovered_camera(self, url: str):
        if url.startswith("rtc://"):
            if self.app_mode == "camera":
                self._snack("Switch to Inputter Mode to receive a camera.")
                return
            if self.camera_on:
                self._stop_camera()
            self.camera_source = url
            self._start_rtc_video("receive", url[6:])
            return
        # Discovery announces Stat Tracker devices; /video requires approval.
        # Use the same handshake as manual Connect before opening the stream.
        self.camera_source_url_field.value = url
        try:
            self.page.run_task(self._pairing_connect_flow_async, url, True)
        except Exception as ex:
            self.camera_error = f"Could not start pairing: {ex}"
            self._full_refresh()
    ...
    async def _pairing_connect_flow_async(self, url: str, require_pairing: bool = False):
        ...
        if not result or result.get("status") != "pending":
            if require_pairing:
                self.camera_error = (
                    "Could not start pairing with the Camera Mode device. "
                    "Keep Start Streaming enabled on that device and try again.")
                self._full_refresh()
                return
            # Not a Stat Tracker Camera Mode device (or unreachable) — no
            # pairing protocol to speak to. Connect directly, unverified.
            self.camera_source = url
            # Always (re)start — not just when camera_on was already
            # True. This "Connected" message promises a live stream is
            # now showing; only starting the capture loop when the
            # camera happened to already be on before this call is what
            # made a first-time connect silently do nothing (source got
            # set, success message shown, but _start_camera() never ran)
            # while the exact same action worked fine as a *second*
            # connect attempt, because by then camera_on was already True
            # from the first, silently-incomplete one.
            if self.camera_on:
                self._stop_camera()
            self._start_camera()
            self._full_refresh()
        ...
        if approved:
            self.camera_source = url
            # Same fix as the unverified-connect branch above — always
            # (re)start regardless of whether camera_on happened to
            # already be True. This is the actual bug behind "Android
            # says paired and connected, but Windows doesn't show
            # anything": the success message was unconditional, but
            # actually starting the stream wasn't.
            if self.camera_on:
                self._stop_camera()
            self._start_camera()
            self._full_refresh()
    ...
    _LOCAL_PREVIEW_MIN_INTERVAL = 1.0 / 60.0  # ceiling for the on-screen UI push
    ...
    async def _camera_capture_loop_async(self):
        ...
        while not self._camera_capture_stop.is_set() and self._cv2_cap:
            ...
            # the loop itself. _encode_frame() itself has no cap and
            # always runs at the camera's max native rate — it stashes
            # every frame into _latest_jpeg_frame (see below) for the
            # MJPEG server regardless of what the local UI does with it.
            data_uri = await asyncio.to_thread(self._encode_frame, cv2)
            if data_uri:
                # Throttle only the on-screen preview's update() call to a
                # 60fps ceiling. update() is a real UI round-trip and can
                # take noticeably longer than a frame interval — calling
                # it unconditionally on every frame serializes that cost
                # into this same loop and was silently capping the whole
                # capture rate (encode included) below what the camera
                # and MJPEG server could otherwise sustain. Skipping the
                # UI push (but not the encode/stream update above) on
                # frames that arrive faster than 60fps keeps this loop —
                # and therefore the stream feeding a connected device —
                # running at the camera's real speed instead of the UI's.
                now = time.monotonic()
                if now - last_ui_update >= self._LOCAL_PREVIEW_MIN_INTERVAL:
                    last_ui_update = now
            ...
            # supports (up to the 60fps request set on the capture device
            # in _start_camera). asyncio.sleep(0) just yields control back
            # to the event loop between iterations — it adds no real delay.
            await asyncio.sleep(0)
    ...
    def _camera_capture_loop(self):
        ...
        while not self._camera_capture_stop.is_set() and self._cv2_cap:
            ...
            if data_uri:
                # Same 60fps UI-push ceiling as the async loop above — see
                # its comment for why this matters for keeping the actual
                # capture/stream rate from being dragged down by update().
                now = time.monotonic()
                if now - last_ui_update >= self._LOCAL_PREVIEW_MIN_INTERVAL:
                    last_ui_update = now
                    self.camera_image.src = data_uri
    ...
    def _open_edit_match_dialog(self, _=None):
        ...
        school_chips = school_quick_pick_row(away_f, self.page, err_field=err,
                                           schools=self._database_school_names())
        ...
        rank_f = ft.Dropdown(label="Team / Age Group", text_size=12, color=TEXT,
                             ...
                             options=[ft.dropdown.Option(r) for r in TEAM_RANK_OPTIONS])
        ...
        def _save_edit(_):
            if not any(item is m for item in self.matches):
                err.value = 'This game changed while you were editing. Close and reopen it to use the latest version.'
                self.page.update()
                return
        ...
        dlg = ft.AlertDialog(
            modal=True, bgcolor=SURFACE, scrollable=True,
            ...
            content=ft.Container(width=440,
                content=ft.Column([away_f, school_chips, loc_f, date_f, rank_f, sport_f, err], spacing=10, tight=True)),
            actions=[
    ...
    def _open_new_match_dialog(self, _=None):
        if not self._database_token:
            self._snack('Sign in before creating a match.')
            self._open_database_dialog()
            return
        ...
        date_f = ft.TextField(label="Match date (YYYY-MM-DD)", value=datetime.now().strftime("%Y-%m-%d"),
                              ...
                              on_focus=self._mark_input_focused, on_blur=self._mark_input_blurred)
        loc_f = ft.Dropdown(label="Venue", value="Home", text_size=12, color=TEXT,
                            bgcolor=SURFACE, border_color=BORDER,
                            options=[ft.dropdown.Option("Home"), ft.dropdown.Option("Away")])
        rank_f = ft.Dropdown(label="Team / Age Group", text_size=12, color=TEXT,
        ...
        def _create(_):
            ...
            nm = Match(id=f"match-{uuid.uuid4().hex[:8]}", sport=sport_f.value or "SOCCER",
                       ...
                       location=loc_f.value if loc_f.value in ("Home", "Away") else "Home",
        ...
        dlg = ft.AlertDialog(
            ...
            content=ft.Container(width=440,
                content=ft.Column([home_locked, away_f, school_chips, date_f, loc_f, rank_f, sport_f, err],
