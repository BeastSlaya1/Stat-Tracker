...
# ═══════════════════════════════════════════════════════════════════════════════
class StatTrackerApp(DatabaseSyncMixin):
    ...
    def _build_camera_mode_screen(self):
        ...
        if self.is_web:
            return ft.Container(content=ft.Column([
                txt("Browser Camera Mode", size=22, color=TEXT, weight=ft.FontWeight.BOLD),
                txt("Send your iPhone, iPad or browser camera to another device.", size=14, color=TEXT),
                txt("Open the camera link, sign in with your staff account, and choose Send this camera. "
                    "On the receiving device, open the same link and enter the code. "
                    "Approve the receiving device on your camera screen.", size=13, color=MUTED),
                ft.Button("Open browser camera", icon=ft.Icons.VIDEOCAM,
                          url="https://stat-tracker-sync.joshuapieterse1.workers.dev/camera"),
                txt("Keep the camera page open and use the same Wi-Fi. Video opens in a separate browser page.",
                    size=12, color=MUTED),
                ft.Button("Switch to Inputter Mode", on_click=self._switch_to_inputter_mode),
            ], spacing=16, scroll=ft.ScrollMode.AUTO), padding=20, expand=True)
    ...
    def _build_url_connect_row(self):
        ...
        return ft.Column([
            ft.Button("Receive iPhone / browser camera", icon=ft.Icons.OPEN_IN_BROWSER,
                      url="https://stat-tracker-sync.joshuapieterse1.workers.dev/camera?mode=receive",
                      tooltip="Opens a paired browser video page alongside your match workspace"),
            txt("Browser camera video opens in a separate page. Android app streams connect below.",
                size=10, color=MUTED),
            ft.Text("On iPhone/iPad, enter the stream URL shown on the other device.",
                    size=11, color=MUTED2) if self.is_ios else ft.Row([discover_btn], spacing=6),
            ft.Column(result_rows, spacing=4, visible=not self.is_ios),
            ft.Row([
    ...
    def _scan_for_wireless_cameras(self, _=None):
        ...
        if self.is_ios or self.wireless_discovering:
            return
