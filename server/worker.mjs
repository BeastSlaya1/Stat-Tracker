import {cameraPage, cameraScript} from './camera-page.mjs';
...
async function handle(request, env) {
  ...
  if (request.method === 'GET' && (route === '/camera' || route === '/camera/app.js')) {
    return new Response(route === '/camera' ? cameraPage : cameraScript, {headers:{
      'Content-Type':route === '/camera' ? 'text/html; charset=utf-8' : 'application/javascript; charset=utf-8',
      'Cache-Control':'no-store', 'Referrer-Policy':'no-referrer',
      'Content-Security-Policy':"default-src 'self'; script-src 'self'; style-src 'unsafe-inline'; media-src 'self' blob:; connect-src 'self'; frame-ancestors 'none'; base-uri 'none'; form-action 'self'",
