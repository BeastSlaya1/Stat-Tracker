import {accountRoutes, catalogRoutes} from './management.mjs';
...
async function staff(request, db) {
  ...
  return db.prepare(`SELECT u.id, u.email, u.display_name, u.role FROM sessions s
...
async function handle(request, env) {
  ...
  if (request.method === "POST" && path === "/login") {
    ...
    return json({token, expires_at: expiresAt, user: {id: user.id, email, display_name: user.display_name, role: user.role}});
  }
  ...
  if(path === '/password' && request.method === 'POST'){
    ...
    if(typeof b.current_password!=='string'||b.current_password.length>128||typeof b.new_password!=='string'||b.new_password.length<12||b.new_password.length>128)return json({error:'Enter your current password and a new password of 12–128 characters.'},400);
    const key='password-change:'+user.id,now=Date.now();
    const attempt=await db.prepare(`INSERT INTO login_attempts(key,count,reset_at) VALUES(?,1,?)
      ON CONFLICT(key) DO UPDATE SET count=CASE WHEN reset_at<=? THEN 1 ELSE count+1 END,
      reset_at=CASE WHEN reset_at<=? THEN excluded.reset_at ELSE reset_at END RETURNING count`).bind(key,now+600000,now,now).first();
    if(attempt.count>10)return json({error:'Too many attempts. Try again in ten minutes.'},429);
    const account=await db.prepare('SELECT password_salt,password_hash FROM users WHERE id=? AND enabled=1').bind(user.id).first();
    if(!account||!equal(await passwordHash(b.current_password,account.password_salt),account.password_hash))return json({error:'Your current password is incorrect.'},400);
    if(b.current_password===b.new_password)return json({error:'Choose a different new password.'},400);
    const salt=random(),hash=await passwordHash(b.new_password,salt),session=await digest(request.headers.get('Authorization').slice(7));
    await db.batch([
      db.prepare(`UPDATE users SET password_salt=?,password_hash=? WHERE id=? AND password_hash=? AND enabled=1
        AND EXISTS(SELECT 1 FROM sessions WHERE token_hash=? AND user_id=? AND expires_at>?)`).bind(salt,hash,user.id,account.password_hash,session,user.id,Date.now()),
      db.prepare('DELETE FROM sessions WHERE user_id=? AND token_hash<>? AND EXISTS(SELECT 1 FROM users WHERE id=? AND password_hash=?)').bind(user.id,session,user.id,hash),
      db.prepare('DELETE FROM login_attempts WHERE key=? AND EXISTS(SELECT 1 FROM users WHERE id=? AND password_hash=?)').bind(key,user.id,hash)
    ]);
    const saved=await db.prepare('SELECT password_hash FROM users WHERE id=?').bind(user.id).first();
    if(!saved||!equal(saved.password_hash,hash))return json({error:'Account access changed. Sign in again before retrying.'},409);
    return json({ok:true});
  }
  if(path === '/me' && request.method === 'GET')return json({user});
  if(path.startsWith('/accounts'))return accountRoutes(request,db,user,bodyOf,passwordHash,random);
  if(path.startsWith('/catalog/manage'))return catalogRoutes(request,db,user,bodyOf);
  ...
  if (request.method === "GET" && path === "/matches") {
    ...
    const result = user.role === 'user'
      ? await db.prepare("SELECT * FROM matches WHERE id>? AND owner_id=? ORDER BY id LIMIT 100").bind(after,user.id).all()
      : await db.prepare("SELECT * FROM matches WHERE id>? ORDER BY id LIMIT 100").bind(after).all();
  ...
  if (request.method === "PUT" && path.startsWith("/matches/")) {
    ...
    if(existing && user.role === 'user' && existing.owner_id !== user.id)return json({error:'You can only change matches you created.'},403);
    ...
    const updated = body.base_version === 0
      ? await db.prepare(`INSERT INTO matches (id,body,deleted,mutation_id,updated_by,updated_at,owner_id)
          VALUES (?,?,?,?,?,?,?) ON CONFLICT(id) DO NOTHING RETURNING version`).bind(id,...values,user.id).first()
    ...
    if (!updated) {
      ...
      if(current && user.role === 'user' && current.owner_id !== user.id)return json({error:'You can only change matches you created.'},403);
