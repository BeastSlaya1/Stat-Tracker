...
class _StcCameraPreviewControlState extends State<StcCameraPreviewControl> {
  ...
  void _onCameraImage(CameraImage image) {
    ...
    // NOT the same "+2" formula used below for the RotatedBox around
    // the preview widget — that value was empirically fitted (see the
    // long comment on that one) to correct an already-rendered Texture
    // this plugin auto-rotates in its own way. Raw CameraImage bytes
    // from startImageStream() are a completely different thing: they
    // come straight off the sensor, unrotated, and Android's own
    // documented convention (CameraCharacteristics.SENSOR_ORIENTATION)
    // is exactly this — rotate the raw image clockwise by
    // sensorOrientation degrees to match the device's natural
    // orientation. No device-specific fitting needed or wanted here;
    // reusing the widget's offset was the actual bug that produced a
    // sideways broadcast image.
    final quarterTurns = controller == null
        ? 0
        : (controller.description.sensorOrientation ~/ 90) % 4;
    ...
    if (controller != null && controller.value.isInitialized) {
      ...
      final quarterTurns =
          !kIsWeb && defaultTargetPlatform == TargetPlatform.android
              ? ((controller.description.sensorOrientation ~/ 90) + 2) % 4
              : 0;
